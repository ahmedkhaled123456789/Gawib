...
const ProductRow = ({ product, index }) => {
  return (
    <tr key={product.id}>
      ...
      <td className="px-4 py-2 text-gray-700">
        <div className="w-72">{product.description}</div>
      </td>
      <td className="px-4 py-2 text-gray-700">
        <div className="w-72">{product.description}</div>
      </td>
    ...
    product.points === 200
      ...
      <td className="px-4 py-2 text-gray-700">
        <div className="w-20">{"سنوات 2"}</div>
      </td>
      <td className="px-4 py-2 text-gray-700">
        <div className="w-20">{"محمد الناصر"}</div>
      </td>
      <td className="px-4 py-2 text-gray-700">
      ...
      <td className="px-4 py-2">
        <div className="flex  items-center justify-center w-24 gap-2">
          <span className=" text-[#085E9C] border border-[#085E9C] rounded px-4 py-1 ">
            نشر
          </span>
        </div>
      </td>
            ...
            <tbody className="divide-y text-center divide-gray-200">
              {games?.data?.length > 0 ? (
                games.data
                  ...
                  .map((game, index) => (
                    <ProductRow key={game.id} product={game} index={index} />
                  ))
              ) : (
                <tr>
