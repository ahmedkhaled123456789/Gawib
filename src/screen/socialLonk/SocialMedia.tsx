...
const SocialMedia = () => {
  ...
  const handleSave = () => {
    const selectedPlatforms = platforms.filter(
      (platform) => platform.checked && platform.url.trim()
    );

    console.log("Selected Social Media:", selectedPlatforms);
    alert(
      `تم حفظ ${selectedPlatforms.length} من منصات التواصل الاجتماعي بنجاح!`
    );
  };
              ...
              socialLinks.map((data) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={data.url}
                    onChange={(e) =>
                      updateUrl(data.id.toString(), e.target.value)
                    }
                    onFocus={() =>
                      !data.is_active && toggleCheckbox(data.id.toString())
                    }
